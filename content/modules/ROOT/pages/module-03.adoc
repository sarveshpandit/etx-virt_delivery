...
. First, change network configuration of the Fedora VMs to make their IP addresses static. This is required, because the policy will block traffic from the DHCP server to the VMs.
...
[source,sh]
----
$ nmcli con mod 'Wired connection 1' ipv4.method manual ipv4.addresses 192.168.64.36/18
