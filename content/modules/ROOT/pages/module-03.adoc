...
. Navigate to the *VirtualMachines* list within the `vmexamples` namespace, and click on the *+2 more* link in the *IP addresses* column for each of your Fedora VMs. Make a note of the IP addresses within the *192.168.64.0/18* subnet.
+
image::module-03/000-vm-ips.png[link=self, window=blank, width=100%]
+
...
[source,sh]
----
[lab-user@bastion ~]$ ping -c3 192.168.64.28
...
[lab-user@bastion ~]$ ping -c3 192.168.64.29
