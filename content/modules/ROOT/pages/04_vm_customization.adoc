...
Pre-configured Red Hat virtual machine templates are listed in the *Virtualization* -> *Templates* page.
These templates are available for different versions of Red Hat Enterprise Linux, Fedora, CentOS, Microsoft Windows Desktop, and Microsoft Windows Server editions.
Each Red Hat virtual machine template is preconfigured with an operating system image, the default settings for the operating system, the flavor (CPU and memory), and the workload type (server).
...
By default pre-configured templates provided by Red Hat OpenShift Virtualization cannot be customized.
However you can clone a template and make adjustments to it for your particular workload in order to make it easier to request specific types of virtual machines for specific workloads.
In this section of the lab we are going to do just this, by creating a template that will provide a database server on demand for our end users.
...
. This will take you to the *Details* page for the template where we will be able to customize some options.
Start by finding the CPU and Memory near the bottom of the page, and click on the pencil icon to edit it.
+
image::module-04/05_Clone_Details.png[link=self, window=blank, width=100%]

. A new window will pop out where you can edit the amount of CPU and Memory.
For our XL template set the value of CPUs to 4, and Memory to 16 GiB, and click the *Save* button.
+
...
(Do not delete the existing contents.)
+
...
This represents one way to install an operating system to a virtual machine that takes advantage of the ability to source disks from many locations, including a web server, object storage, or other persistent volumes in the cluster.

This process can be streamlined after the initial operating system installation by creating a template from the virtual machine.
...
. Note that the *Use this disk as a boot source* option has been automatically enabled.
There seems to be a bug in 4.18 on this page. When you click on the *Save* button, the page crashes.
Just exit the screen without clicking on the `Save` button for now.
+
...
. Downloading the Windows ISO will take several minutes.
When the VM in in a *Provisioning* state, you can monitor the download by clicking on the status.
+
image::module-04/provisioning.png[link=self, window=blank, width=100%]

. Eventually, the Virtual VM will enter the *Running* status.
Switch to the *Console* tab immediately, because the Windows installation ISO requires you to press a key to actually boot from the ISO (because Windows).
+
...
.. In the VM firmware interface, select the *Reset* option to reboot the VM.
When the ISO bootloader starts, press a key to begin the Windows installation.
+
image::module-04/boot_mgr.png[link=self, window=blank, width=100%]

. Once the VM installation process is complete, go ahead and power it off with the stop button.
(You may wish to check the VM's CPU utilization on the *Metrics* tab to ensure that the VM has completed all of its first boot setup.)
We can now take a snapshot of the root disk, and use that to create a bootable volume template that we can use to boot other Windows 2k19 machines.
+
...
. Find the *windows* PVC that is listed and using the three-dot menu on the right select *Clone PVC*.
On the menu that pops up, name the new PVC *windows-2k19-template*.
+
...
In this section we learned how to clone and customize and existing template to create one that can be used for specific workloads like databases.
We also learned how to configure one of the existing Windows templates that exists without a boot source, and automate it's installation process, so we can create future templates by cloning the installation PVC that is created with that VM.
